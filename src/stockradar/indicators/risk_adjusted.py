...
def compute_beta_adjusted_rs(
    ...
    start_anchor = nth_business_anchor(merged.index, run_anchor, return_window)
    if start_anchor is None:
        return pd.Series([None], index=[run_anchor])

    idx = pd.to_datetime(merged.index).sort_values().unique()
    beta_end = start_anchor
    beta_start = nth_business_anchor(idx, beta_end, beta_window)
    ...
    stock_cumret = anchored_return(merged["stock_close"], run_anchor, start_anchor)
    bench_cumret = anchored_return(merged["bench_close"], run_anchor, start_anchor)
...
def compute_information_ratio(
    ...
    if run_anchor is None:
        return pd.Series([None], index=[pd.Timestamp(run_date)])
    start_anchor = nth_business_anchor(merged.index, run_anchor, window)
    if start_anchor is None:
        return pd.Series([None], index=[run_anchor])
