...
現時点では、公式 API 系の情報源だけでは、機能構成・網羅性・安定性の面で本候補に必要な入力を満たせない可能性が高い。したがって、初期版は公式 API のみを前提にせず、非公式 API、公開ページからの取得、最小限のスクレイピングを含む取得層を現実的な選択肢として扱う。

設計上の最優先制約:

- 情報ソース側のサーバー負荷を低く抑える。
- ソースデータ、PDF 本文、HTML、加工前テキストなどの原情報を再配布しない。

取得層の実装原則:

- Adapter / Protocol で取得手段を分離し、公式 API・非公式 API・スクレイピング・手動投入を差し替え可能にする。
- robots、利用規約、再配布条件を勘案する場合は、非商用の研究用途を基準に判断する。
- レート制限、指数バックオフ、User-Agent 明示、差分取得、日次・月次の取得上限を設ける。
- 同一 URL / 同一開示への再取得を避けるため、取得メタデータとハッシュのみを cache / manifest に保持する。
- 保存・配布するのは、判定に必要な構造化特徴量、理由コード、参照 ID、取得日時、ハッシュ等に限定する。
- 生データの長期保存が必要な場合も、ローカルまたは非公開ストレージ内の検証用途に限定し、日次・月次成果物には含めない。
- 取得失敗・取得抑止・robots/規約判断によるスキップは、flags / manifest に記録する。

初期版で想定する入力:

- JPX の公式情報: 上場銘柄マスター、整理・監理・特別注意等の制度情報。
- EDINET 相当の開示情報: 有価証券報告書、監査報告書、売上高、粗利益、営業 CF、純資産、発行済株式数、セグメント情報、GC 記載、監査意見。
- TDnet 相当の適時開示情報: MS ワラント、第三者割当、月間行使状況、資金使途変更、監査法人変更、決算延期、新規事業開始、定款変更、社名変更等。
- PoC では手動投入 JSONL / CSV、または取得済みメタデータのみを Fake source として使い、判定ロジックを先に固定する。

初期版で避けること:

- 高頻度・全量・無制限のクロール。
- 原文 PDF、HTML、本文抽出テキスト、未加工 CSV など、ソースデータに近い情報の成果物同梱・再配布。
- 取得可否や利用条件の判断をコード内に埋め込んだまま監査不能にすること。
- PDF 本文の曖昧な自然言語解析だけに依存した除外判定。
...
- 入力データの取得方式について、低負荷・非再配布・非商用研究用途の前提を満たす運用条件が最低1つ確定している。
