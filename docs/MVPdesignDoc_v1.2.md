# MVP Design Document v1.2

（v1.1 から更新。成果物管理の用語を実装に合わせて整理）
...
Universe とは、「**ある日付において評価対象とする銘柄集合**」を指す。
...
### 7.4 成果物管理の軸（Pipeline / Visibility）

成果物の管理は、次の2軸を直交させて扱う。

* **Pipeline（確定性）**
  * Staging: 検証ゲート未通過の一時成果物（`data/output/staging/<run_id>/`）
  * Committed: 検証ゲート通過済み成果物（`data/output/latest/LATEST_RUN_ID.txt` が参照点）
  * Release 添付・GDrive 反映は Committed 後に実施する
* **Visibility（公開範囲）**
  * Work（0011_work）: 内部運用向け。CSV 等の中間/最終成果物を月/日フォルダで管理
...
* 状態を持たない
* ログは成果物と同列に扱う
* staging -> latest の原子更新を徹底する
* 外部公開（Release / Drive）は Committed 状態のみを対象とする
