...
| キー                 | 判定                    | 出力語    | 需給含意        |
| ------------------ | --------------------- | ------ | ----------- |
| `RANGE_VERY_LARGE` | `q_sr >= 0.95`        | `<極大>`   | 需給大変動       |
| `RANGE_LARGE`      | `0.85 <= q_sr < 0.95` | `<大>`    | 強い方向性または乱高下 |
| `RANGE_NORMAL`     | `0.40 <= q_sr < 0.85` | （表示省略） | 平常域の消化      |
| `RANGE_SMALL`      | `0.15 <= q_sr < 0.40` | `<小>`    | 吸収・拮抗       |
| `RANGE_VERY_SMALL` | `q_sr < 0.15`         | `<極小>`   | 強い拮抗        |
...
- `[窓]` は **上窓＋／下窓＋** のみ。窓が無い場合は出力しない。
- `[サイズ]` は **`<極大>`／`<大>`／`<小>`／`<極小>`**（角括弧はサイズ部の視認用。v1.2 までの「極大の／大きな／小さな／極小の」から短縮）。`RANGE_NORMAL` はサイズ語を出力しない。
...
| キー         | 判定（v1.1以降）        | 出力語 |
| ---------- | ----------------- | --- |
| `GAP_UP`   | `前日の最低値 > 当日の最高値` | 上窓＋ |
| `GAP_DOWN` | `前日の最高値 < 当日の最低値` | 下窓＋ |


**判定ロジック**:

1. 前日と当日それぞれで、OHLCの最低値と最高値を確認
2. 前日の最高値よりも当日の最低値の方が高い場合 → **下窓＋**（GAP_DOWN）
3. 前日の最低値よりも当日の最高値の方が低い場合 → **上窓＋**（GAP_UP）
...
| キー                   | 判定                          | 出力語   |
| -------------------- | --------------------------- | ----- |
| `WICK_BOTH_LONG`     | `ur >= 0.45 AND dr >= 0.45` | 長上下ヒゲ |
| `WICK_BOTH_PRESENT`  | `ur >= 0.25 AND dr >= 0.25` | 上下ヒゲ  |
| `WICK_UPPER_LONG`    | `ur >= 0.45`                | 長上ヒゲ  |
| `WICK_LOWER_LONG`    | `dr >= 0.45`                | 長下ヒゲ  |
...
- `price_text`（例: `S高タッチ疑い` / `上窓＋<極大>長上ヒゲ陰線` / `<大>丸坊主陽線` / `レンジ0`）
- `volume_text`（将来実装予定）
...
- 断定語は使用しない
- 価格挙動と需給含意は分離
- サイズ語の **`<` `>`** は表示上の区切り。**Webに生のHTMLとして挿入する場合**は `&lt;` / `&gt;` でエスケープするか、テキストノードとして設定すること（未エスケープだとブラウザがタグと誤解する可能性）。代替として **全角括弧 `＜極大＞`** も可だが、CSV/XLSX実装と一致させるならASCIIの `<` `>` を正とする。
...
## 9.3 v1.3変更点（`price_text` 表記の短縮）

- **窓**: v1.2 までの `上窓つき` / `下窓つき` → v1.3 以降は **`上窓＋` / `下窓＋`**
- **サイズ（相対）**: v1.2 までの `極大の` / `大きな` / `小さな` / `極小の` → v1.3 以降は **`<極大>` / `<大>` / `<小>` / `<極小>`**（先頭の `<` 終端の `>` は視認用。文字列の一部として連結する）
- **LONG系ヒゲ**: v1.2 までの `長い上ヒゲ` 等 → v1.3 以降は **`長上ヒゲ` / `長下ヒゲ` / `長上下ヒゲ`**（「い」を省略）
- ラベル（`candle_labels`）の英語トークン（`GAP_UP` / `RANGE_LARGE` 等）は変更なし
