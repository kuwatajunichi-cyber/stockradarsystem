...
| metric_set_members | FK-ordered junction (set ↔ metric version) | Members are retained; REST insert draft-set only (trigger) |
...
| derived_object_index | Committed R2 snapshot / series metadata | Audit and active objects; snapshots immutable (one committed per object_key) and require SHA256 `layer1_input_fingerprint`; series regeneratable (pending per object_key, prior committed orphaned on commit); REST insert pending-only, commit via RPC |
| latest_derived_observations | Latest values per instrument and active set | Latest row only; service_role SELECT only (mode-aware writes via follow-up RPC at 4.5c) |
