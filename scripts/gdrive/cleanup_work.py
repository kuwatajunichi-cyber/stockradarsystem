"""
0011_work 配下で 3 か月より古い月フォルダ（YYYY-MM）を削除する CLI。
Drive 凍結解除後に有効化する想定。
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Drive 0011_work の 3 か月より古い月フォルダを削除する。"
    )
    parser.add_argument(
        "--today",
        metavar="YYYY-MM-DD",
    ...
    print(f"Drive クリーンアップ完了（削除数: {deleted}）", file=sys.stderr)
